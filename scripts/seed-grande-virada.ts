...
async function main() {
  ...
  const usersResult = await sql`SELECT id, organization_id FROM users LIMIT 1`;
  const { id: userId, organization_id: orgId } = usersResult[0];
  ...
  await sql`
    ...
    VALUES (${quizId}, ${orgId}, ${userId}, ${"A Grande Virada"}, ${null}, ${{}}::jsonb, ${"draft"}, NOW(), NOW())
  `;
